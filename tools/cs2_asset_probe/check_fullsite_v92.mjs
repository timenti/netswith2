...
for(const view of [{name:'desktop',width:1600,height:900},{name:'mobile',width:390,height:844}]){
 ...
 await page.waitForFunction(()=>{const rs=performance.getEntriesByType('resource');return rs.filter(x=>x.name.includes('/aist-fast/')).length>=4&&rs.some(x=>x.name.includes('/aist-film/site-runtime.bundle.js'))},{timeout:15000});
 await page.waitForTimeout(100);const preloadAt=Date.now()-navStart;
 const preload=await page.evaluate(()=>performance.getEntriesByType('resource').filter(x=>x.name.includes('/aist-fast/')||x.name.includes('/aist-film/site-runtime.bundle.js')).map(x=>({name:x.name.split('/').pop(),duration:Math.round(x.duration),transferSize:x.transferSize,encodedBodySize:x.encodedBodySize})));
 await page.evaluate(()=>performance.clearResourceTimings());
 const clickAt=Date.now();await page.locator('a[href="/projects/aist/"]').first().click();
 await page.waitForFunction(()=>location.pathname.replace(/\/$/,'')==='/projects/aist'&&document.getElementById('aist-film-v92')?.dataset.assetReady==='1',{timeout:6000});
 const aistReadyAfterClick=Date.now()-clickAt;await page.waitForTimeout(50);
 const state=await page.evaluate(()=>{const f=document.getElementById('aist-film-v92'),c=f?.querySelector('canvas');return{path:location.pathname,assetReady:f?.dataset.assetReady||'',routeMountDelayMs:+(f?.dataset.routeMountDelayMs||0),importMs:+(f?.dataset.importMs||0),runtimeMountMs:+(f?.dataset.runtimeMountMs||0),assetLoadMs:+(f?.dataset.assetLoadMs||0),canvas:{w:c?.width||0,h:c?.height||0},postClickResources:performance.getEntriesByType('resource').filter(x=>x.name.includes('/aist-fast/')||x.name.includes('/aist-film/site-runtime.bundle.js')).map(x=>({name:x.name.split('/').pop(),duration:Math.round(x.duration),transferSize:x.transferSize,encodedBodySize:x.encodedBodySize}))}});
...
if(report.views.some(v=>v.state.assetReady!=='1'||v.state.canvas.w===0||v.errors.length||v.failed.some(x=>x.url.includes('/aist-fast/')||x.url.includes('/aist-film/'))||v.state.postClickResources.some(r=>r.transferSize>0)))process.exit(2);
